...
## Node Stuff™️
* use [nvm](https://github.com/nvm-sh/nvm?tab=readme-ov-file#installing-and-updating) to install and manage npm/node
* **Windows**
	* **requires admin**
	```pwsh
...
## WSL
* edit your `/etc/wsl.conf` to remove windows paths from WSL - the windows node/npm will cause problems in the linux filesystem
```
[automount]
...
```
* you make specific modification to the `$PATH` variable in your `.profile`, such as sourcing vscode which is designed to work across WSL and Windows
* install `nvm` separately in WSL by following the [nvm install instructions](https://github.com/nvm-sh/nvm?tab=readme-ov-file#installing-and-updating)
* when developing in VSCode, you can set remote-specific settings by running the editor command (`Ctrl+Shift+P`) and searching for "Preferences: Open Remote Settings (JSON) (WSL: Ubuntu)".
